...
namespace UMS2026
{
    ...
    {
        // Compatibility aliases for Designer-generated control names
        private System.Windows.Forms.DataGridView takesDataGridView => this.enrollmentDataGridView;
        private System.Windows.Forms.BindingSource takesBindingSource => this.enrollmentBindingSource;
        private UniversityDataSetTableAdapters.takesTableAdapter takesTableAdapter => this.enrollmentTableAdapter;
        private System.Windows.Forms.TextBox iDTextBox => this.studentIdTextBox;
        private System.Windows.Forms.TextBox course_idTextBox => this.courseIdTextBox;
        private System.Windows.Forms.TextBox sec_idTextBox => this.secIdTextBox;
        ...
        {
            InitializeComponent();
            this.enrollmentDataGridView.CellClick += enrollmentDataGridView_CellClick;
            // Wire buttons in case Designer didn't set them
        ...
        {
            this.Validate();
            this.enrollmentBindingSource.EndEdit();
        ...
        private void EnrollmentForm_Load(object sender, EventArgs e)
        {
            ...
            this.enrollmentTableAdapter.Fill(this.universityDataSet.takes);
        ...
        private void LoadEnrollments()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(conStr))
                {
                    ...
                    da.Fill(dt);
                    this.enrollmentBindingSource.DataSource = dt;
                    this.enrollmentDataGridView.DataSource = this.enrollmentBindingSource;
                    this.enrollmentBindingSource.ResetBindings(false);
                }
            }
        ...
        private void enrollmentDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0) return;
            var row = enrollmentDataGridView.Rows[e.RowIndex];
            studentIdTextBox.Text = row.Cells[0].Value?.ToString() ?? "";
            courseIdTextBox.Text = row.Cells[1].Value?.ToString() ?? "";
            secIdTextBox.Text = row.Cells[2].Value?.ToString() ?? "";
